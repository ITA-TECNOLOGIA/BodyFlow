...
import copy
import os
from human_pose_estimation.models.predictors_3d.mhformer.mhformer_model import Model
...
class MHFormer(HPE3D):
    def __init__(self, window_length, developer_parameters):
        ...
        if developer_parameters is None:
            ...
            model_path = os.path.join(developer_parameters['models_path'], f"mhformer_model_{window_length}.pth")
    ...
    def get_3d_keypoints(self, img_w, img_h, bodyLandmarks2d, timestamp, input_2D_no, frame, bboxes):
        ...
        if False:
            # Transformation with respect the bottle
            bottle_coordinates_tapon = np.array([[1360, 2300]])
            bottle_coordinates_base = np.array([[1540, 2300]])
            bottle_pixel_size = np.linalg.norm(bottle_coordinates_base - bottle_coordinates_tapon)
            bottle_size_meters = 0.32
            # Assuming a bottle in meters is the same as the distance from th wrist to the elbow
            dist_reference = np.linalg.norm(post_out[13,:] - post_out[12,:])
            # Reescaling
            scale = bottle_size_meters/dist_reference
            post_out = post_out * scale

            hips = bodyLandmarks2d._hips.coordinates
            translation = (hips - np.array([bottle_coordinates_base[0][1], bottle_coordinates_base[0][0]])) * (0.32 /  bottle_pixel_size)
            translation_3d = np.array([translation[0], translation[1], 0])
            post_out = post_out + translation_3d
