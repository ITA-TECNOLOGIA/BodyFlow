...
class ITAdata():
    ...
    def load_data_from_csv(self):
        df = pd.read_csv(self.path_dataset, low_memory=False)#, nrows = 20000)
    ...
    def normalization(self):
        ...
        self.df.iloc[:, np.r_[self.har_dict_csv['pose_c1_2d'], self.har_dict_csv['pose_c2_2d']]] = min_max_scaler.fit_transform(self.df.iloc[:, np.r_[self.har_dict_csv['pose_c1_2d'], self.har_dict_csv['pose_c2_2d']]])

    def featureSelection(self):
        df_info = self.df.iloc[:, np.r_[self.har_dict_csv['labels']]]
        if self.features == 'imus' or self.features == 'ankle' or self.features == 'all':
            df_f = self.df.iloc[:, np.r_[self.har_dict_csv[self.features]]]     
        elif self.features == '2d':
            df_f = self.df.iloc[:, np.r_[self.har_dict_csv['pose_c1_2d'], self.har_dict_csv['pose_c2_2d']]]
        elif self.features == '3d':
            df_f = self.df.iloc[:, np.r_[self.har_dict_csv['pose_c1_3d'], self.har_dict_csv['pose_c2_3d']]]
        ...
        if self.features == "2d" or self.features == "3d" or self.features == "all":
            ...
            df_info = self.df.iloc[:, np.r_[self.har_dict_csv['labels']]]
            df_imus = self.df.iloc[:, np.r_[self.har_dict_csv['imus']]]
            
            self.df = pd.concat([df_info, df_pose], axis=1)
            if self.features == "all":
                self.df = pd.concat([df_info, df_imus, df_pose], axis=1)
